...
unique_elements, counts_elements = np.unique(yhatDT, return_counts=True)
...
#KNN
neigh = KNeighborsClassifier(n_neighbors = 2).fit(X_train,y_train.ravel())
...
print(pd.crosstab(y_test.ravel(), yhatkNN.ravel(), rownames = ['True'], colnames = ['Predicted'], margins = True))
...
colors_2 = ['lightcoral','salmon', 'indianred', 'mistyrose', 'firebrick', 'lightsalmon']

bar_viz = viz_df_count.plot (kind = 'barh', color = [colors_2], figsize=(10, 8), alpha = 0.75 )
plt.xlabel ("Frequency", fontsize = 14, weight = 'bold', labelpad = 20)
plt.ylabel (None)
plt.title ("Accident Causes - Seattle, Washington", fontsize = 16, weight = 'bold')
plt.tight_layout()

colors = {'Adverse Lighting Conditions':'lightcoral', 'Adverse Road Conditions':'salmon', 'Adverse Weather Conditions':'indianred',
'Over-speeding':'mistyrose', 'Under the Influence':'firebrick', 'Inattentive Driving':'lightsalmon'}
